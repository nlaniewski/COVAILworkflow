...
```{r protocol.version}
protocol.hash<-tools::md5sum(
  list.files(
    system.file("protocols",package="COVAILworkflow"),
    pattern="COVAIL_protocol_staining_AIM.Rmd",
    full.names = TRUE
  )
)
protocol.hash.alias<-paste(substr(protocol.hash,1,4),substr(protocol.hash,29,32),sep="_")
```
### Version: `r protocol.hash.alias`

# Reagents

```{r reagents.table}
COVAILworkflow:::reagents.table(reagent.type = 'AIM',table.type = 'kable')
```

# Buffers

```{r buffers.table}
COVAILworkflow:::buffers.table(buffers.type = 'AIM')
```
...
COVAILworkflow:::panel.kable()$buffer$AIM.extracellular
COVAILworkflow:::panel.kable()$mix$AIM.extracellular
```
...
#. $\square$ Pool each **Barcode Block** into respective, pre-labeled 15 mL tube
    + NAMING CONVENTION: *Study_Batch_PANEL_BLOCK#*; 
    + Example: *COVAIL_001_AIM_BLOCK1*
    + Example: *COVAIL_002_AIM_BLOCK2*
...
#. $\square$ *Vortex-Genie Shaker*: 20 minutes, room-temperature
  ...
  **Use SB (without nuclease) for washes**
    
#. $\square$ *WASH*: 2 mL **SB**; spin 800xg, 5 minutes, room-temperate; vacuum aspirate supernatant
#. $\square$ *WASH*: 3 mL **SB**; spin 800xg, 5 minutes, room-temperate; vacuum aspirate supernatant
